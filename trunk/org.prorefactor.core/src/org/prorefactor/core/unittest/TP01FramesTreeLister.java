...
public class TP01FramesTreeLister extends JPNodeLister {
	...
	public TP01FramesTreeLister(JPNode topNode, String outfilename, TokenTypesReader typesReader) {
	...
	protected String generateLineText(JPNode node) {
		...
		if (node instanceof FieldRefNode) fieldRefNode(buff, (FieldRefNode)node);
		if (node.getType()==TokenTypes.INPUT) fieldContainer(buff, node);
	...
	private void fieldContainer(StringBuffer buff, JPNode node) {
		...
		buff
			.append(spacer)
			.append(typesReader.getName(fieldContainer.getProgressType()))
			.append("=")
			;
