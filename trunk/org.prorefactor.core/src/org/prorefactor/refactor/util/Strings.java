 ...
 * I'm not sure why that class isn't a core Eclipse class,
 * would have been handy there.
 * 
 * Note that some Joanju methods have been added to the top of the class.
 */
...
public class Strings {
	 ...
	 * Replaces the first line's indent with the newIndent.
	 * For the rest of the lines, strips the minimum indent of all lines
	 * other than the first line, then adds the newIndent.
	 * Skips the first line, because it may be from a selection beginning mid-line.
	 * (Input a leading newline to avoid this behaviour.)
	 * This function won't work very well if the code's indent is not consistent for
	 * tabs and spaces, because it counts characters to remove - not "width".
	 */
	public static String changeIndent(String text, String newIndent, String lineDelim) {
		String [] lines = convertIntoLines(text);
		lines[0] = trimIndents(lines[0], 1);
		StringBuffer buff = new StringBuffer(newIndent + lines[0]);
		if (lines.length>1) {
			// Second line is the basis.
			int min = getIndentString(lines[1]).length();
			int indent;
			// Start at second line.
			for (int i = 1; i < lines.length; i++) {
				if (lines[i].trim().length() == 0) continue;
				indent = getIndentString(lines[i]).length();
				if (indent < min) min = indent;
			}
			for (int i = 1; i < lines.length; i++) {
				if (lines[i].trim().length() == 0) {
					buff.append(lineDelim + lines[i]);
					continue;
				} 
				buff.append(lineDelim + newIndent + lines[i].substring(min));
			}
		}
	...
	public static String getIndentString(String line) {
		...
		for (int i = 0; i < size; i++) {
			if (isIndentChar(line.charAt(i))) end = i + 1;
			else break;
		}
		if (end == 0) return "";
		else if (end == size) return line;
