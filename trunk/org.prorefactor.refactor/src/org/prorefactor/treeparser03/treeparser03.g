...
class TreeParser03 extends TreeParser;
options {
...
definebrowsestate
	:	#(	DEFINE (#(NEW (GLOBAL)? SHARED ) | SHARED)? BROWSE id:ID
...
definebufferstate
	:	#(	DEFINE (#(NEW (GLOBAL)? SHARED ) | SHARED)? BUFFER id:ID
			FOR rec:RECORD_NAME (PRESELECT)? (label_constant)? (#(FIELDS (field)* ))? state_end
		)
	;

definebuttonstate
	:	#(	DEFINE (#(NEW (GLOBAL)? SHARED ) | SHARED)? BUTTON id:ID
			(	AUTOGO
			|	AUTOENDKEY
			|	DEFAULT
			|	color_expr
...
defineframestate
	:	#(	DEFINE (#(NEW (GLOBAL)? SHARED ) | SHARED)? FRAME id:ID
			(form_item)*
			(	#(HEADER (display_item)+ )
...
defineimagestate
	:	#(	DEFINE (#(NEW (GLOBAL)? SHARED ) | SHARED)? IMAGE id:ID
			(	#(LIKE field (VALIDATE)?)
			|	imagephrase_opt 
			|	sizephrase
			|	color_expr
...
definemenustate
	:	#(	DEFINE (#(NEW (GLOBAL)? SHARED ) | SHARED)? MENU id:ID
...
definequerystate
	:	#(	DEFINE (#(NEW (GLOBAL)? SHARED ) | SHARED)? QUERY id:ID
...
definerectanglestate
	:	#(	DEFINE (#(NEW (GLOBAL)? SHARED ) | SHARED)? RECTANGLE id:ID
			(	NOFILL
...
definestreamstate
	:	#(	DEFINE (#(NEW (GLOBAL)? SHARED ) | SHARED)? STREAM id:ID state_end )
	;

definesubmenustate
	:	#(	DEFINE (#(NEW (GLOBAL)? SHARED ) | SHARED)? SUBMENU id:ID
			(menu_opt)* (menu_list_item)* state_end
		)
	;
...
disablestate
	:	#(DISABLE (UNLESSHIDDEN)?
		(#(ALL (#(EXCEPT (field)*))?) | (form_item)+)? (framephrase)? state_end )
	;
...
enablestate
	:	#(ENABLE (UNLESSHIDDEN)?
		(#(ALL (#(EXCEPT (field)*))?) | (form_item)+)? (#(IN_KW WINDOW expression))?
		(framephrase)? state_end )
	;
...
functionstate
	...
	;
function_param
	:	#(BUFFER (id:ID)? FOR rec:RECORD_NAME (PRESELECT)? )
...
recordphrase
	:	#(	rec:RECORD_NAME // is root node, so can't refer to record_name function
